...
Plot.plot({
  marks: [
    Plot.barY(dataset, {x: 'name', y: 'value', fill: 'name', fillOpacity: 0.5, stroke: 'name'}),
  ]
})
```
...

{
  ...
  const nbBar = dataset.length; // Le nombre de barre
  const spaceBetweenBar = 5; // L'espace entre les barres
  const widthBar = width / nbBar; // La largeur de chaque barre
  ...
  // Création d'un groupe ('g') pour contenir les barres
  const bars = svg.append('g')
  			.attr('class', 'bars');
  
  // Lier les données au graphique a créer
  bars.selectAll('rect')
  	.data(dataset)
    .enter()
    .append('rect')
    .attr('x', (d, i) => i * widthBar) // Position sur l'axe horizontal
    .attr('y', (d) => height - d) // Position sur l'axe vertical
    .attr('width', widthBar - spaceBetweenBar) // Largeur de la barre
    .attr('height', (d) => d); // Hauteur de la barre
...
- Pour créer les deux dernières visualisation, nous avons fait correspondre des **valeurs** de données à des **propriétés visuelles**.
...
- [Observable Plot](https://observablehq.com/plot/) est une bibliothèque open source de visualisation développée notamment par Mike Bostock (le créateur de D3.js) et Philippe Rivière (un des principaux contributeurs de D3.js et auteur du site [visioscarto.net](https://visioscarto.net)).
